...
for i in `cat $PBS_NODEFILE`; do
	...
	if [ ${node_rank} -eq 0 ]; then
		...
		ssh $i server_ip=$server_ip server_port=$server_port num_clients=$num_clients nproc_per_node=$((e-s+1)) num_threads=$num_threads nnodes=$nnodes lr=$lr dataset=$dataset iid=$iid split=$split method=$method d=$d target_acc=$target_acc round=$round repeat=$repeat selection_ratio=$selection_ratio /scratch/paop01a16/FedMPI/server.sh &
	fi
	if [ ${node_rank} -ne 0 ]; then
		num_threads=`echo $num_threads_per_node | tr -d ' '`
		ssh $i server_ip=$server_ip server_port=$server_port num_clients=$num_clients nproc_per_node=$((e-s+1)) num_threads=$num_threads nnodes=$nnodes lr=$lr dataset=$dataset iid=$iid split=$split method=$method d=$d target_acc=$target_acc round=$round repeat=$repeat node_rank=$node_rank selection_ratio=$selection_ratio /scratch/paop01a16/FedMPI/client.sh &
	fi
