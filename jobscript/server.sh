...
singularity exec ../FLenv.sif torchrun --nnodes=$nnodes --nproc_per_node=$((nproc_per_node+1)) --node_rank=0 --rdzv_id=0 --rdzv_endpoint=$server_ip:$server_port --master_addr=$server_ip --master_port=$server_port Start.py --selection_ratio=$selection_ratio --round=$round --batch_size=32 --local_epochs=5 --lr=$lr --target_acc=$target_acc --num_threads=$num_threads --dataset=$dataset --iid=$iid --split=$split --method=$method --d=$d --wandb_on=True --project=SemiAsync --entity=yumiri --group=$dataset --name=$method --cluster_type=KISTI --repeat=$repeat
